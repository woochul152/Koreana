...
//		/*Sample data ends*/
		...
		try {
		    ...
		    String sql = "SELECT * FROM Auctions WHERE AccountNo = ? AND AirlineID = ? AND FlightNo = ? AND SeatClass = ? " +
		                 "ORDER BY Date DESC LIMIT 1";
		    
		    PreparedStatement pstmt = con.prepareStatement(sql);
		    pstmt.setInt(1, AccountNo);
		    pstmt.setString(2, AirlineID);
		    pstmt.setInt(3, FlightNo);
		    pstmt.setString(4, SeatClass);
	...
	public List<Auctions> getAllBids(int AccountNo, String AirlineID, int FlightNo, String SeatClass) {
		...
		try {
			...
			ResultSet rs = st.executeQuery(
					"SELECT * FROM Auctions WHERE AccountNo = " + AccountNo + 
		            " AND AirlineID = '" + AirlineID + "' AND FlightNo = " + FlightNo + 
